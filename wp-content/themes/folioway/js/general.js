...
jQuery(document).ready(function(){
    ...
    jQuery('#header .menu li').hover(function() {	
		...
		function() {	
            submenu = jQuery('> .sub-menu', this);
            top_distance = jQuery(this).parent().is('#header .menu') ? '15px' : '0';
    ...
    left_slots = 0;
	var duration = 'slow',easing = 'easeOutQuad';
	var dNext=jQuery('.works .next,');
	var dPrev=jQuery('.works .prev,');
	var slidetiphide=function(){
		if(belt_slots<=4){
			dNext.addClass("novisible");
			dPrev.addClass("novisible");
			return false;
			}
		dNext.removeClass("novisible");
		dPrev.removeClass("novisible");
		if(left_slots>=belt_slots-4){
			dNext.addClass("novisible")
			}else if(left_slots<=0){
				dPrev.addClass("novisible");
				}
		};
	slidetiphide();
	var next=function(){
		if(left_slots>=belt_slots-4) return false;
        left_slots += 4;
		belt.animate({marginLeft: '-=998px'}, duration, easing);
		slidetiphide();
		};
    var prev=function(){
		if(left_slots<=0) return false;
		belt.animate({marginLeft: '+=998px'}, duration, easing);
        left_slots -= 4;
		slidetiphide();
		};
    jQuery('.works .next').click(next);
    jQuery('.works .prev').click(prev);
